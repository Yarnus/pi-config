...
export default function askUserQuestion(pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			if (!ctx.hasUI) {
				return unavailableResult(params.question, mode, "ask_user_question requires TUI or RPC dialog UI", context);
			}

			if (mode !== "text" && ctx.mode !== "tui") {
				return unavailableResult(params.question, mode, "Choice questions require TUI mode; retry without options for free-text input", context);
			}
